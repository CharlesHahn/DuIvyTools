...
将xpm文件转化为M\*N的dat文件。目前只支持分立数据的xpm文件。输出文件的第一行为注释，依次为数据第一行的标题，数据第二行的标题，以及数据M\*N矩阵的标题。
...
氢键占有率图：
![hbond occupancy](static/hbond_0.png)

氢键距离随时间变化图：
![hbond distance](static/hbond_1.png)

氢键角度随时间变化图：
![hbond angle](static/hbond_2.png)
